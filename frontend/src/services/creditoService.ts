...
function normalizeCreditoDetalle(raw: any): CreditoDetalle {
  ...
  return {
    ...
    estadisticas: {
      ...
      umbralRenovacion: estadisticasRaw.umbralRenovacion ?? estadisticasRaw.umbral_renovacion ?? 0,
      pagosFaltantesRenovacion: estadisticasRaw.pagosFaltantesRenovacion ?? estadisticasRaw.pagos_faltantes_renovacion ?? 0,
      abonosParcialesPendientes: estadisticasRaw.abonosParcialesPendientes ?? estadisticasRaw.abonos_parciales_pendientes ?? 0,
      saldoAbonosParciales: estadisticasRaw.saldoAbonosParciales ?? estadisticasRaw.saldo_abonos_parciales ?? 0,
    },
...
export const creditoService = {
  ...
  calcularProducto: (capital: number, tipoPago?: 'DIARIO' | 'SEMANAL', plazo?: number) =>
    api.get('/creditos/calcular', { params: { capital, tipoPago, plazo } }).then((r) => normalizeProductoCalculo(r.data)),

  calcularOpciones: (capital: number, tipoPago?: 'DIARIO' | 'SEMANAL'): Promise<ProductoCalculo[]> =>
    api.get('/creditos/opciones-calculo', { params: { capital, tipoPago } })
      .then((r) => (r.data ?? []).map(normalizeProductoCalculo)),
  ...
  aprobarCredito: (id: number, data: { montoAprobado: number; plazo?: number; observaciones?: string }) =>
